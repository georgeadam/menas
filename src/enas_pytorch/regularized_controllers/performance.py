...
class PerformanceRegularizedController(torch.nn.Module):
    """Based on
    ...
    def __init__(self, args):
        ...
        self.performance_predictor = torch.nn.Linear(args.controller_hid, 1)
        self._performance_predictor = self.performance_predictor
        ...
        self.replay_buffer = {"hiddens": [], "performances": []}
    ...
    def sample(self, batch_size=1, with_details=False, save_dir=None, return_hidden=False,
        ...
        if len(self.replay_buffer["hiddens"]) >= 1:
            predicted_performances = self.performance_predictor(torch.cat(self.replay_buffer["hiddens"][-100:]))
            mses = F.mse_loss(predicted_performances, torch.Tensor(self.replay_buffer["performances"][-100:]).cuda())
            if len(mses.shape) == 0:
                mses = [mses.view(1)]
        else:
            mses = [torch.Tensor([0.0])]
